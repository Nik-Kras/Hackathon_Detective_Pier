		...
		SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : COLOR
			{
				...
				half2 perspectiveCorrection = half2(1.0f * (0.5 - i.uv.x) * i.uv.y, 0.0f);
				half2 offset;
				...
				float isMask = tex2D(_Mask, i.uv.xy);
				offset = isMask * offset;
				
				float2 adjusted = i.uv.xy + offset / _DisplacementAmountDivider;
				fixed4 col = tex2D(_MainTex, adjusted);
				fixed4 colAdj = col*float4(AdjustContrastCurve(_Tint, 1 - Grayscale(col)).rgb, 1);

				//float isMask = tex2D(_Mask, colAdj.x) == (1,1,1,1);
				//colAdj = (1 - isMask) * colAdj + isMask * float4(1,1,1,1);
					
				//foam thresholding
				if ((abs((offset.x - 0.5) / _DisplacementAmountDivider) > _FoamThreshold &&
						abs((offset.y - 0.5) / _DisplacementAmountDivider) > _FoamThreshold) || i.uv.y <
					_EdgeFoamThreshold
					* (offset.x - 0.5) / _DisplacementAmountDivider)
